...
func buildCLI(cli *CLIConfig, logger logging.Logger) *cobra.Command {
	...
	rootCmd.PersistentFlags().StringVar(&cfg.WatPath, "wat-path", "", "WAT file to send")
	rootCmd.PersistentFlags().StringVar(&cfg.Contract, "contract", "", "Contract to spam")
	rootCmd.PersistentFlags().StringVar(&cfg.ExecMsg, "exec-msg", "", "Conctract Msg")
